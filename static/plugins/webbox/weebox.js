...
(function($) {
	...
	var weebox = function(content, options) {
		...
		this.setCenterPosition = function() {
			...
			pTop = Math.min(pTop, minTop);
			pLeft += (wnd.width() - self.dh.width()) / 2;
			*/
            var wnd = window.parent, doc = $(wnd.document), pTop = doc.scrollTop(), pLeft = doc.scrollLeft();
            pTop += (window.screen.availHeight - 300 - self.dh.height()) / 2;
			pLeft += (window.screen.availWidth - 160 - self.dh.width()) / 2;
